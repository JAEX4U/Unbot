...
def create_code(admin_id: int) -> tuple[str, "datetime"]:
    """Generate a code, store it, and return (code, expires_at)."""
    expires_at = datetime.now(timezone.utc) + timedelta(minutes=config.CODE_EXPIRY_MINUTES)
    with closing(sqlite3.connect(DB_PATH)) as conn:
        code = generate_unique_code(conn)
        conn.execute(
            "INSERT INTO codes (code, created_by, expires_at) VALUES (?, ?, ?)",
            (code, admin_id, expires_at.isoformat()),
        )
        conn.commit()
...
async def gencode(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    code, expires_at = create_code(admin.id)
    ...
    await update.effective_message.reply_text(
        forwardable_code_message(code, expires_at),
        parse_mode=ParseMode.MARKDOWN,
    )
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "admin_gencode":
        code, expires_at = create_code(admin_user.id)
        ...
        await context.bot.send_message(
            ...
            text=forwardable_code_message(code, expires_at),
            parse_mode=ParseMode.MARKDOWN,
        )
        ...
        if not rows:
            ...
            lines = [f"`{c}` — until {datetime.fromisoformat(e).strftime('%H:%M UTC')}" for c, e in rows]
            text = "📄 *Active Codes*\n\n" + "\n".join(lines)
