...
# Display timezone -- all timestamps are stored in UTC internally, but shown
# to admins/users converted to this zone.
DISPLAY_TZ = timezone(timedelta(hours=5, minutes=30))  # IST (India Standard Time)
TZ_LABEL = "IST"


def fmt_time(dt: datetime) -> str:
    """Format a UTC datetime for display in the configured display timezone."""
    return dt.astimezone(DISPLAY_TZ).strftime(f"%I:%M%p {TZ_LABEL}")
...
def escape_markdown(text: str) -> str:
    ...
    (ParseMode.MARKDOWN) so usernames/names with _, *, `, [ don't break parsing."""
    for ch in ("_", "*", "`", "["):
...
def forwardable_code_message(code: str, admin_user, created_at: datetime, expires_at: datetime) -> str:
    ...
    raw_label = admin_user.username and f"@{admin_user.username}" or admin_user.full_name
    admin_label = escape_markdown(raw_label)
    return (
        "🔐 *Your Access Code*\n\n"
        f"Code: `{code}`\n"
        f"Generated by: {admin_label}\n"
        f"Generated at: {fmt_time(created_at)}\n"
        f"Valid until: *{fmt_time(expires_at)}* "
...
async def admin_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if query.data == "admin_gencode":
        ...
        # Don't re-edit the panel message here -- its text/keyboard would be
        # identical to what's already shown, which Telegram rejects as a
        # no-op edit ("Message is not modified").
        ...
        if not rows:
            ...
            lines = [f"`{c}` — until {fmt_time(datetime.fromisoformat(e))}" for c, e in rows]
            text = "📄 *Active Codes*\n\n" + "\n".join(lines)
...
async def error_handler(update: object, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if isinstance(err, BadRequest) and "Message is not modified" in str(err):
        # Harmless: user tapped a button that would produce identical content.
        return
